...
if [ -f /etc/wireguard/wg0.conf ]; then
    # Prompt to check if it should be overwritten
    read -p "The WireGuard server configuration file (wg0.conf) already exists. Do you want to overwrite it? (yes/no): " overwrite
    if [ "$overwrite" != "yes" ]; then
        echo "Exiting without overwriting the configuration file."
        exit 1
    fi
fi
